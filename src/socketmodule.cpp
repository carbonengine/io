	...
	internal_setblocking( PySocketSockObject* s, int block )
{
	...
	u_long arg;
#endif
#if !defined( MS_WINDOWS ) && !( ( defined( HAVE_SYS_IOCTL_H ) && defined( FIONBIO ) ) )
...
static int
	internal_select( PySocketSockObject* s, int writing, _PyTime_t interval, int connect )
{
	int n;
#ifdef HAVE_POLL
				  ...
				  _PyTime_t timeout )
{
	...
	while( 1 )
	{
		...
		if( s->sock_timeout > 0 && ( CHECK_ERRNO( EWOULDBLOCK ) || CHECK_ERRNO( EAGAIN ) ) )
		{
		...
		Py_END_ALLOW_THREADS
           ...
           *are* safe regarding their return value, ie. that a
           ...
           outcome of the first call. */
			RELEASE_GETADDRINFO_LOCK if( error )
		{
			set_gaierror( error );
			return -1;
		}
	...
	makebdaddr( bdaddr_t* bdaddr )
{
	...
	sprintf( buf, "%02X:%02X:%02X:%02X:%02X:%02X", bdaddr->b[5], bdaddr->b[4], bdaddr->b[3], bdaddr->b[2], bdaddr->b[1], bdaddr->b[0] );
	return PyUnicode_FromString( buf );
}
#endif
	...
	makesockaddr( SOCKET_T sockfd, struct sockaddr* addr, size_t addrlen, int proto )
{
	...
	case AF_UNIX: {
		...
		if( linuxaddrlen > 0 && a->sun_path[0] == 0 )
...
#ifdef CAN_ISOTP
		case CAN_ISOTP: {
			return Py_BuildValue( "O&kk", PyUnicode_DecodeFSDefault, ifname, a->can_addr.tp.rx_id, a->can_addr.tp.tx_id );
		}
#endif /* CAN_ISOTP */
		default: {
			return Py_BuildValue( "(O&)", PyUnicode_DecodeFSDefault, ifname );
		}
		}
	}
	...
	idna_converter( PyObject* obj, struct maybe_idna* data )
{
	...
	{
		PyErr_Format( PyExc_TypeError, "str, bytes or bytearray expected, not %s", obj->ob_type->tp_name );
		return 0;
	}
...
static int
	getsockaddrarg( PySocketSockObject* s, PyObject* args, struct sockaddr* addr_ret, int* len_ret, const char* caller )
{
	...
	case AF_INET: {
		...
		result = setipaddr( host.buf, (struct sockaddr*)addr, sizeof( *addr ), AF_INET );
		idna_cleanup( &host );
		if( result < 0 )
			return 0;
	...
	case AF_INET6: {
		...
		result = setipaddr( host.buf, (struct sockaddr*)addr, sizeof( *addr ), AF_INET6 );
		idna_cleanup( &host );
		if( result < 0 )
			return 0;
		if( port < 0 || port > 0xffff )
	...
	case AF_BLUETOOTH: {
		...
		case BTPROTO_L2CAP: {
			...
			if( !PyArg_ParseTuple( args, "si", &straddr, &_BT_L2_MEMB( addr, psm ) ) )
			{
		...
		case BTPROTO_RFCOMM: {
			...
			if( !PyArg_ParseTuple( args, "si", &straddr, &_BT_RC_MEMB( addr, channel ) ) )
			{
				PyErr_Format( PyExc_OSError,
							  "%s(): wrong format",
							  caller );
	...
	case AF_CAN:
		...
		case CAN_ISOTP: {
			...
			if( !PyArg_ParseTuple( args, "O&kk", PyUnicode_FSConverter, &interfaceName, &rx_id, &tx_id ) )
				return 0;
	...
	case PF_SYSTEM:
		...
		case SYSPROTO_CONTROL: {
			...
			if( PyUnicode_Check( args ) )
			{
				...
				if( !PyArg_Parse( args, "O&", PyUnicode_FSConverter, &ctl_name ) )
				{
					return 0;
				}
				...
				strncpy( info.ctl_name, PyBytes_AS_STRING( ctl_name ), sizeof( info.ctl_name ) );
				Py_DECREF( ctl_name );
			...
			else if( !PyArg_ParseTuple( args, "II", &( addr->sc_id ), &( addr->sc_unit ) ) )
			{
				PyErr_Format( PyExc_TypeError,
	...
	sock_accept_impl( PySocketSockObject* s, void* data )
{
	...
	if( accept4_works != 0 )
	{
		ctx->result = accept4( s->sock_fd, addr, paddrlen, SOCK_CLOEXEC );
	...
	sock_setsockopt( PySocketSockObject* s, PyObject* args )
{
	...
	if( s->sock_family == AF_VSOCK )
	{
		...
		if( PyArg_ParseTuple( args, "iiK:setsockopt", &level, &optname, &vflag ) )
		{
			// level should always be set to AF_VSOCK
			res = setsockopt( s->sock_fd, level, optname, (void*)&vflag, sizeof vflag );
			goto done;
		}
		return NULL;
	}
	...
	{
		res = setsockopt( s->sock_fd, level, optname, (char*)&flag, sizeof flag );
		goto done;
	}
	...
	if( PyArg_ParseTuple( args, "iiO!I:setsockopt", &level, &optname, Py_TYPE( Py_None ), &none, &optlen ) )
	{
		...
		res = setsockopt( s->sock_fd, level, optname, NULL, (socklen_t)optlen );
		goto done;
	}
	...
	sock_getsockopt( PySocketSockObject* s, PyObject* args )
{
	...
	if( buflen == 0 )
	{
		...
		if( s->sock_family == AF_VSOCK )
		{
			...
			res = getsockopt( s->sock_fd, level, optname, (void*)&vflag, &flagsize );
			if( res < 0 )
				return s->errorhandler();
		...
		res = getsockopt( s->sock_fd, level, optname, (char*)&flag, &flagsize );
		if( res < 0 )
			return s->errorhandler();
	...
	internal_connect( PySocketSockObject* s, struct sockaddr* addr, int addrlen, int raise )
{
	...
	if( raise )
	{
		...
		if( sock_call_ex( s, 1, sock_connect_impl, NULL, 1, NULL, s->sock_timeout ) < 0 )
			return -1;
	}
	else
	{
		/* socket.connect_ex() returns the error code on error */
		if( sock_call_ex( s, 1, sock_connect_impl, NULL, 1, &err, s->sock_timeout ) < 0 )
			return err;
	}
	return 0;
}
	...
	sock_getsockname( PySocketSockObject* s, PyObject* Py_UNUSED( ignored ) )
{
	...
	return makesockaddr( s->sock_fd, SAS2SA( &addrbuf ), addrlen, s->sock_proto );
}
	...
	sock_recvfrom_impl( PySocketSockObject* s, void* data )
{
	...
	if( ctx->len > INT_MAX )
		ctx->len = INT_MAX;
	ctx->result = recvfrom( s->sock_fd, ctx->cbuf, (int)ctx->len, ctx->flags, SAS2SA( ctx->addrbuf ), ctx->addrlen );
#else
	ctx->result = recvfrom( s->sock_fd, ctx->cbuf, ctx->len, ctx->flags, SAS2SA( ctx->addrbuf ), ctx->addrlen );
#endif
	return ( ctx->result >= 0 );
}
 ...
 */
static Py_ssize_t
	sock_recvfrom_guts( PySocketSockObject* s, char* cbuf, Py_ssize_t len, int flags, PyObject** addr )
{
	sock_addr_t addrbuf;
	socklen_t addrlen;
 ...
 */
static PyObject*
	sock_recvmsg_guts( PySocketSockObject* s, struct iovec* iov, int iovlen, int flags, Py_ssize_t controllen, PyObject* ( *makeval )(ssize_t, void*), void* makeval_data )
{
	sock_addr_t addrbuf;
		 ...
		 cmsgh = CMSG_NXTHDR( &msg, cmsgh ) )
	{
		...
		bytes = PyBytes_FromStringAndSize( (char*)CMSG_DATA( cmsgh ),
										   cmsgdatalen );
		tuple = Py_BuildValue( "iiN", (int)cmsgh->cmsg_level, (int)cmsgh->cmsg_type, bytes );
		if( tuple == NULL )
			goto err_closefds;
	...
	retval = Py_BuildValue( "NOiN",
							...
							cmsg_list,
							(int)msg.msg_flags,
							makesockaddr( s->sock_fd, SAS2SA( &addrbuf ), ( ( msg.msg_namelen > addrbuflen ) ? addrbuflen : msg.msg_namelen ), s->sock_proto ) );
	if( retval == NULL )
		goto err_closefds;
	...
	sock_recvmsg( PySocketSockObject* s, PyObject* args )
{
	...
	retval = sock_recvmsg_guts( s, &iov, 1, flags, ancbufsize, &makeval_recvmsg, &buf );
	Py_XDECREF( buf );
	return retval;
}
	...
	sock_recvmsg_into( PySocketSockObject* s, PyObject* args )
{
	...
	if( !PyArg_ParseTuple( args, "O|ni:recvmsg_into", &buffers_arg, &ancbufsize, &flags ) )
		return NULL;
	...
	if( nitems > 0 && ( ( iovs = PyMem_New( struct iovec, nitems ) ) == NULL || ( bufs = PyMem_New( Py_buffer, nitems ) ) == NULL ) )
	{
		PyErr_NoMemory();
		goto finally;
	}
	...
	retval = sock_recvmsg_guts( s, iovs, nitems, flags, ancbufsize, &makeval_recvmsg_into, NULL );
finally:
	for( i = 0; i < nbufs; i++ )
	...
	sock_sendto_impl( PySocketSockObject* s, void* data )
{
	...
	if( ctx->len > INT_MAX )
		ctx->len = INT_MAX;
	ctx->result = sendto( s->sock_fd, ctx->buf, (int)ctx->len, ctx->flags, SAS2SA( ctx->addrbuf ), ctx->addrlen );
#else
	ctx->result = sendto( s->sock_fd, ctx->buf, ctx->len, ctx->flags, SAS2SA( ctx->addrbuf ), ctx->addrlen );
#endif
	return ( ctx->result >= 0 );
}
...
static int
	sock_sendmsg_iovec( PySocketSockObject* s, PyObject* data_arg, struct msghdr* msg, Py_buffer** databufsout, Py_ssize_t* ndatabufsout )
{
	...
	sock_sendmsg( PySocketSockObject* s, PyObject* args )
{
	...
	if( !PyArg_ParseTuple( args, "O|OiO:sendmsg", &data_arg, &cmsg_arg, &flags, &addr_arg ) )
	{
		return NULL;
	}
	...
	{
		if( !getsockaddrarg( s, addr_arg, SAS2SA( &addrbuf ), &addrlen, "sendmsg" ) )
		{
			goto finally;
		}
	...
	sock_sendmsg_afalg( PySocketSockObject* self, PyObject* args, PyObject* kwds )
{
	...
	if( !PyArg_ParseTupleAndKeywords( args, kwds, "|O$O!y*O!i:sendmsg_afalg", keywords, &data_arg, &PyLong_Type, &opobj, &iv, &PyLong_Type, &assoclenobj, &flags ) )
	{
		return NULL;
	}
	...
	sock_ioctl( PySocketSockObject* s, PyObject* arg )
{
	...
	case SIO_RCVALL: {
		...
		if( !PyArg_ParseTuple( arg, "kI:ioctl", &cmd, &option ) )
			return NULL;
		if( WSAIoctl( s->sock_fd, cmd, &option, sizeof( option ), NULL, 0, &recv, NULL, NULL ) == SOCKET_ERROR )
		{
			return set_error();
		}
	...
	case SIO_KEEPALIVE_VALS: {
		...
		if( !PyArg_ParseTuple( arg, "k(kkk):ioctl", &cmd, &ka.onoff, &ka.keepalivetime, &ka.keepaliveinterval ) )
			return NULL;
		if( WSAIoctl( s->sock_fd, cmd, &ka, sizeof( ka ), NULL, 0, &recv, NULL, NULL ) == SOCKET_ERROR )
		{
			return set_error();
		}
		return PyLong_FromUnsignedLong( recv );
	...
	sock_share( PySocketSockObject* s, PyObject* arg )
{
	...
	Py_END_ALLOW_THREADS if( result == SOCKET_ERROR ) return set_error();
...
static PyMethodDef sock_methods[] = {
...
#endif
#if defined( MS_WINDOWS )
	{ "share", (PyCFunction)sock_share, METH_VARARGS, sock_share_doc },
#endif
	{ "listen", (PyCFunction)sock_listen, METH_VARARGS, listen_doc },
	{ "recv", (PyCFunction)sock_recv, METH_VARARGS, recv_doc },
	{ "recv_into", (PyCFunction)(void ( * )( void ))sock_recv_into, METH_VARARGS | METH_KEYWORDS, recv_into_doc },
	{ "recvfrom", (PyCFunction)sock_recvfrom, METH_VARARGS, recvfrom_doc },
	{ "recvfrom_into", (PyCFunction)(void ( * )( void ))sock_recvfrom_into, METH_VARARGS | METH_KEYWORDS, recvfrom_into_doc },
	{ "send", (PyCFunction)sock_send, METH_VARARGS, send_doc },
	...
	{ "getblocking", (PyCFunction)sock_getblocking, METH_NOARGS, getblocking_doc },
...
#ifdef CMSG_LEN
	{ "recvmsg", (PyCFunction)sock_recvmsg, METH_VARARGS, recvmsg_doc },
	{
		"recvmsg_into",
		(PyCFunction)sock_recvmsg_into,
		METH_VARARGS,
		recvmsg_into_doc,
	},
	{ "sendmsg", (PyCFunction)sock_sendmsg, METH_VARARGS, sendmsg_doc },
#endif
#ifdef HAVE_SOCKADDR_ALG
	...
	sock_initobj( PyObject* self, PyObject* args, PyObject* kwds )
{
	...
	if( !PyArg_ParseTupleAndKeywords( args, kwds, "|iiiO:socket", const_cast<char**>( keywords ), &family, &type, &proto, &fdobj ) )
		return -1;
...
#endif
	{
		if( PySys_Audit( "socket.__new__", "Oiii", s, family, type, proto ) < 0 )
		{
			return -1;
		}
	}
	...
	if( fdobj != NULL && fdobj != Py_None )
	{
		...
		if( PyBytes_Check( fdobj ) )
		{
			...
			if( PySys_Audit( "socket()", "iii", info.iAddressFamily, info.iSocketType, info.iProtocol ) < 0 )
			{
				return -1;
			}

			Py_BEGIN_ALLOW_THREADS
				fd = WSASocketW( FROM_PROTOCOL_INFO, FROM_PROTOCOL_INFO, FROM_PROTOCOL_INFO, &info, 0, WSA_FLAG_OVERLAPPED );
			Py_END_ALLOW_THREADS if( fd == INVALID_SOCKET )
			{
				set_error();
				return -1;
			}
...
#endif
		{
			...
			if( type == -1 )
			{
				...
				if( getsockopt( fd, SOL_SOCKET, SO_TYPE, (char*)&tmp, &slen ) == 0 )
				{
					type = tmp;
				}
				else
			...
			if( proto == -1 )
				{
					int tmp;
					socklen_t slen = sizeof( tmp );
					if( getsockopt( fd, SOL_SOCKET, SO_PROTOCOL, (void*)&tmp, &slen ) == 0 )
					{
						proto = tmp;
					}
					else
...
#define WSA_FLAG_NO_HANDLE_INHERIT 0x80
			...
			Py_BEGIN_ALLOW_THREADS if( support_wsa_no_inherit )
			{
				fd = WSASocketW( family, type, proto, NULL, 0, WSA_FLAG_OVERLAPPED | WSA_FLAG_NO_HANDLE_INHERIT );
				if( fd == INVALID_SOCKET )
				{
	...
	socket_gethostname( PyObject* self, PyObject* unused )
{
	...
	Py_END_ALLOW_THREADS if( res < 0 ) return set_error();
	buf[sizeof buf - 1] = '\0';
	...
	socket_sethostname( PyObject* self, PyObject* args )
{
	...
	{
		PyErr_Clear();
		if( !PyArg_ParseTuple( args, "O&:sethostname", PyUnicode_FSConverter, &hnobj ) )
			return NULL;
		flag = 1;
	}
	...
	socket_gethostbyname_ex( PyObject* self, PyObject* args )
{
	...
	Py_END_ALLOW_THREADS
       ...
       access sa_family. */
		sa = SAS2SA( &addr );
	ret = gethost_common( h, SAS2SA( &addr ), sizeof( addr ), sa->sa_family );
#ifdef USE_GETHOSTBYNAME_LOCK
	...
	socket_getservbyname( PyObject* self, PyObject* args )
{
	...
	Py_END_ALLOW_THREADS if( sp == NULL )
	{
	...
	socket_getaddrinfo( PyObject* self, PyObject* args, PyObject* kwargs )
{
	...
	if( PySys_Audit( "socket.getaddrinfo", "OOiii", hobj, pobj, family, socktype, protocol ) < 0 )
	{
		return NULL;
	}
	...
	for( res = res0; res; res = res->ai_next )
	{
		...
		if( addr == NULL )
			goto err;
		single = Py_BuildValue( "iiisO", res->ai_family, res->ai_socktype, res->ai_protocol, res->ai_canonname ? res->ai_canonname : "", addr );
		Py_DECREF( addr );
		if( single == NULL )
			goto err;
	...
	socket_getnameinfo( PyObject* self, PyObject* args )
{
	...
	if( !PyArg_ParseTuple( sa, "si|II;getnameinfo(): illegal sockaddr argument", &hostp, &port, &flowinfo, &scope_id ) )
	{
		return NULL;
	}
	if( flowinfo > 0xfffff )
	...
	error = getnameinfo( res->ai_addr, (socklen_t)res->ai_addrlen, hbuf, sizeof( hbuf ), pbuf, sizeof( pbuf ), flags );
	if( error )
	{
		set_gaierror( error );
		goto fail;
	...
	socket_if_nameindex( PyObject* self, PyObject* arg )
{
	...
	for( ULONG i = 0; i < tbl->NumEntries; ++i )
	{
		...
		if( ( ret = ConvertInterfaceLuidToNameW( &r.InterfaceLuid, buf, Py_ARRAY_LENGTH( buf ) ) ) )
		{
			Py_DECREF( list );
			FreeMibTable( tbl );
			// ret is used instead of GetLastError()
...
#ifdef HAVE_SETHOSTNAME
	{ "sethostname", socket_sethostname, METH_VARARGS, sethostname_doc },
#endif
...
#endif
	{ "getaddrinfo", (PyCFunction)(void ( * )( void ))socket_getaddrinfo, METH_VARARGS | METH_KEYWORDS, getaddrinfo_doc },
...
#if defined( HAVE_IF_NAMEINDEX ) || defined( MS_WINDOWS )
	{ "if_nameindex", socket_if_nameindex, METH_NOARGS, if_nameindex_doc },
	{ "if_nametoindex", socket_if_nametoindex, METH_VARARGS, if_nametoindex_doc },
	{ "if_indextoname", socket_if_indextoname, METH_O, if_indextoname_doc },
#endif
#ifdef CMSG_LEN
	...
	PyInit__socket( void )
{
	...
	socket_gaierror = PyErr_NewException( "socket.gaierror", PyExc_OSError, NULL );
	...
	/* Export C API */
	if( PyModule_AddObject( m, PySocket_CAPI_NAME, PyCapsule_New( &PySocketModuleAPI, PySocket_CAPSULE_NAME, NULL ) ) != 0 )
		return NULL;
...
/**/
	...
	PyModule_AddIntConstant( m, "INADDR_ALLHOSTS_GROUP", INADDR_ALLHOSTS_GROUP );
#else
