...
class CarbonBuildWindows(buildName: String, configType: String, preset: String) : BuildType({
    ...
    vcs {
        root(AbsoluteId("Carbon_Io_2_IoFeatureKotlin"),"+:. => %github_checkout_folder%")
    ...
    triggers {
        vcs {
            triggerRules = "+:root=${AbsoluteId("Carbon_Io_2_IoFeatureKotlin").id}:."
    ...
    features {
        pullRequests {
            vcsRootExtId = "${AbsoluteId("Carbon_Io_2_IoFeatureKotlin")}"
            provider = github {
                authType = token {
